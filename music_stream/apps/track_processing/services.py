...
class TrackConvertorHLS:
    ...
    def convert_track(self, track_id: int) -> str:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            cmd = [
                ...
                "hls",
                "-hls_time",
                "4",  # Целевая длительность сегмента
                "-hls_list_size",
    ...
    def _upload_hls_to_minio(self, track: Track, output_dir: Path) -> str:
        ...
        for filename in output_dir.iterdir():
            ...
            content_type = "video/MP2T" if filename.suffix == ".mp4" else "application/vnd.apple.mpegurl"
        ...
        return minio.fetch_presigned_track_hls_playlist_url(playlist_url)
